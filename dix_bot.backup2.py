...
def can_make_request(user_id: int) -> bool:
    ...
    used = get_user_requests_today(user_id)
    return used < limit

def add_referral(referrer_id: int, new_user_id: int) -> None:
    user_referrals[new_user_id] = str(referrer_id)
    if referrer_id in user_limits:
        today = datetime.now().strftime('%Y-%m-%d')
        if user_limits[referrer_id].get('date') == today:
            user_limits[referrer_id]['count'] = max(0, user_limits[referrer_id]['count'] - 1)
    try:
        import asyncio
...
def create_main_menu_keyboard(user_id: int) -> ReplyKeyboardMarkup:
    ...
    row5 = KeyboardButtonRow(buttons=[KeyboardButton('ℹ️ О боте'), KeyboardButton('🔗 Рефералы')])
    sub_row = KeyboardButtonRow(buttons=[KeyboardButton('⭐ Подписка')])
    buttons_rows = [row1, row2, row3, row4, row5, sub_row]
    if user_id == ADMIN_ID:
...
KEYBOARD_BUTTONS: List[str] = [
    ...
    '🔙 Отмена', '🔙 В меню', '🔗 Рефералы', '⭐ Подписка', '🎁 Подарить подписку', '✅ Проверить подписку',
...
async def keyboard_handler(event):
    ...
    if text == '📞 Номер':
        ...
        if check_subscription(user_id):
            await event.respond('<b>✅ У вас активна подписка.</b>\
\
<b>Запросов:</b> безлимитно', parse_mode='html')
        else:
            await event.respond(
                '<b>⭐ ПОДПИСКА</b>\
\
...
'
                f'<a href="https://t.me/{SUBSCRIPTION_CHANNEL.replace("@", "")}">{SUBSCRIPTION_CHANNEL}</a>\
\
'
                '<b>После подписки:</b>\
'
...
'
        ...
        try:
            ...
            try:
                await bot(GetParticipantRequest(SUBSCRIPTION_CHANNEL, user_id))
                user_subscriptions[user_id] = True
                await event.respond('<b>✅ Подписка активирована!</b>\
\
<b>Теперь у вас безлимитные запросы!</b>', buttons=create_main_menu_keyboard(user_id), parse_mode='html')
            except:
                await event.respond('<b>❌ Вы не подписаны на канал.</b>\
\
<i>Подпишитесь: {SUBSCRIPTION_CHANNEL}</i>', parse_mode='html')
        except:
            await event.respond('<b>✅ Подписка активирована!</b>\
\
<b>Теперь у вас безлимитные запросы!</b>', buttons=create_main_menu_keyboard(user_id), parse_mode='html')
        ...
        ref_count = sum(1 for v in user_referrals.values() if v == str(user_id))
        await event.respond(
...
\
'
            f'<b>Перешло по ссылке:</b> {ref_count}\
'
            f'<b>Бонус за каждого:</b> +{REFERRAL_BONUS} запрос\
\
'
            '<i>Отправьте ссылку другу. Когда он зайдёт — вы получите +1 запрос.</i>',
            parse_mode='html'
        )
    
    elif text == '🎁 Подарить подписку':
        if user_id != ADMIN_ID:
            await event.respond('<b>⛔ Только для администратора.</b>', parse_mode='html')
            return
        user_states[user_id] = 'waiting_gift_subscription'
        await event.respond(
            '<b>🎁 ПОДАРИТЬ ПОДПИСКУ</b>\
\
'
            '<b>Отправьте ID пользователя:</b>',
            buttons=create_cancel_keyboard(),
            parse_mode='html'
        )
...
async def incoming_handler(event):
    ...
    if state == 'waiting_phone':
        ...
        if len(cleaned) < 10:
            ...
            return
        if not can_make_request(user_id) and user_id != ADMIN_ID:
            await event.respond(
                '<b>❌ ДОСТИГНУТ ЛИМИТ</b>' + chr(10) + chr(10) +
                '<b>Использовано:</b> ' + str(get_user_requests_today(user_id)) + '/' + str(get_daily_limit(user_id)) + chr(10) +
                '<b>Подпишитесь на</b> ' + SUBSCRIPTION_CHANNEL + chr(10) +
                '<b>или пригласите друга по реферальной ссылке.</b>',
                parse_mode='html'
            )
            return
    ...
    # Обработка рассылки
    elif state == 'waiting_gift_subscription' and user_id == ADMIN_ID:
        if text.isdigit():
            target_id = int(text)
            user_subscriptions[target_id] = True
            user_states[user_id] = None
            await event.respond(f'<b>✅ Подписка подарена пользователю <code>{target_id}</code>!</b>', buttons=create_admin_panel_keyboard(), parse_mode='html')
            try:
                await bot.send_message(target_id, '<b>🎁 Вам подарили подписку!</b>\
\
<b>Теперь у вас безлимитные запросы.</b>', parse_mode='html')
            except:
                pass
        else:
            await event.respond('<b>❌ Неверный ID.</b>', buttons=create_cancel_keyboard(), parse_mode='html')
